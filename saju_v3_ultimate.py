...
일간({r["ilgan"]["stem"]}, {r["ilgan"]["ohaeng"]})의 본질을 자연물에 비유해 생생하게 묘사하세요(예: 갑목은 큰 나무, 계수는 이슬비). 일지({r["pillars"]["day"]["branch"]})가 보여주는 속마음과 겉모습의 차이. 십이운성({r["ilgan"]["twelve_fortune"]})이 말하는 에너지 상태. 어린 시절부터 형성된 기질, 남들이 보는 나 vs 진짜 나. 강점 3가지와 약점 2가지를 구체적으로. (500자 이상)

## ⚖️ 2. 오행의 균형 — 내 안의 다섯 기운
목·화·토·금·수 다섯 기운의 분포를 분석하고, 과한 기운과 부족한 기운이 성격·건강·인간관계에 어떻게 드러나는지 구체적으로. 부족한 기운을 채우는 실천법(음식·색·취미·환경·인간관계)을 재미있고 실용적으로 제안. (450자 이상)

## 🏛️ 3. 격국 — 내 인생의 무대와 役할
{r["yukguk"]}의 의미를 쉽게 풀이하고, 이 사람이 어떤 무대에서 가장 빛나는지, 어떤 방식으로 세상과 관계 맺는지 설명. 이 격국을 가진 역사적 인물이나 유형의 예시를 들어 흥미롭게. (450자 이상)

## 🔑 4. 용신 — 인생을 여는 황금 열쇠
용신({r["yongshin"].get("용신","")})이 왜 이 사람에게 가장 필요한 기운인지 쉽게 설명. 이 기운을 강화하는 직업·환경·습관·인테리어·인간관계를 아주 구체적으로. 기신({r["yongshin"].get("기신","")})은 무엇이고 어떻게 피할지도. (450자 이상)

## 💰 5. 재물과 경제 — 돈의 흐름
재성(財星) 구조를 분석해 평생의 재물 그릇과 흐름을 설명. 사업형 vs 직장형 중 어느 쪽이 맞는지, 투자 성향(공격적/안정적), 큰돈이 들어오는 시기와 조심할 시기, 어떤 분야·방식으로 돈이 들어오는지 구체적으로. 돈 관리 실전 조언. (550자 이상)

## 💼 6. 직업과 성취 — 일에서의 나
관성(官星, 조직·명예)과 식상(食傷, 재능·표현)의 균형을 분석. 가장 잘 맞는 직군 TOP 3와 이유, 조직 생활 vs 프리랜서·창업 적합도, 30대·40대·50대 커리어 흐름과 승진·전환 시기, 직장 내 인간관계 스타일. (550자 이상)

## ❤️ 7. 사랑과 인연 — 마음의 운명
배우자궁인 일지({r["pillars"]["day"]["branch"]}) 해석으로 어떤 배우자·연인과 인연이 깊은지(성격·외모·분위기). 연애 스타일, 결혼 적령기, 결혼 후 관계 모습. {"배우자궁에 충(沖)이 있어 관계의 부침이나 늦은 결혼 가능성. 이를 지혜롭게 다루는 법도 조언. " if spouse_warn else ""}좋은 인연을 만나고 지키는 실전 조언. (550자 이상)

## 🏥 8. 건강 — 몸이 보내는 신호
오행 부족({r["career_health"]["약한오행"]})과 과다로 인해 평생 주의할 신체 부위와 질환을 구체적으로. 현재 대운({cd['age']}~{cd['age']+9}세)에서 특히 신경 쓸 건강 포인트. 체질에 맞는 음식·운동·생활습관을 재미있고 실용적으로. (450자 이상)

## 🌊 9. 현재 대운 — 지금 흐르는 10년의 강물
현재 대운 {cd["stem"]}{cd["branch"]}({cd['age']}~{cd['age']+9}세)이 가져오는 기운과 분위기를 생생하게 묘사. 이 10년의 핵심 키워드 3개. 이 시기에 시작하면 좋은 것 / 정리하면 좋은 것 / 조심할 것. 인생 전체 흐름에서 지금이 어떤 계절(봄·여름·가을·겨울)인지. (550자 이상)

## 📅 10. {cur_year}년 올해 운세 — 열두 달의 지도
올해 전체 운세의 큰 흐름과 분위기(상승/조정/도약/휴식 등). 가장 빛나는 달과 가장 조심할 달을 콕 집어서. 재물·일·연애·건강·이동/이사 각 분야별 올해 포인트. 현재 {cur_month}월부터 향후 8개월의 월별 흐름을 구체적으로 짚어주기. (650자 이상)

## 🔮 11. 다가올 3년 — 미래를 미리 보기
{cur_year+1}년과 {cur_year+2}년의 큰 흐름과 주요 사건 가능성(기회·변화·시련·만남). 이 시기를 위해 지금부터 준비하면 좋은 것. 인생의 터닝포인트가 될 만한 시점. (450자 이상)

## ⭐ 12. 종합 — 당신에게 보내는 편지
지금까지의 분석을 따뜻하게 종합. 이 사주의 가장 빛나는 강점 한 가지를 격려와 함께. 평생 마음에 새기면 좋을 좌우명 같은 조언 한 문장. 행운의 색({", ".join(r["career_health"]["행운색상"][:2])})·방위({r["career_health"]["행운방위"]})·숫자({r["career_health"]["행운숫자"]})를 일상에서 활용하는 재미있는 팁. 마지막은 희망적이고 따뜻한 한마디로 마무리. (500자 이상)
