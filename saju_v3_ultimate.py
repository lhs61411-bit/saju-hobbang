...
try:
    ...
    def lunar_to_solar(year: int, month: int, day: int) -> tuple:
        ...
        return _klc.SolarYear, _klc.SolarMonth, _klc.SolarDay
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
st.set_page_config(
    page_title="천명도 v3 | Ultimate 사주 분석",
    page_icon="☯",
    layout="wide",
...
/* ── 사이드바 내부 텍스트 ── */
section[data-testid="stSidebar"],
section[data-testid="stSidebar"] *:not(button):not(.stButton>button){
...
section[data-testid="stSidebar"] .stCheckbox label{color:#1a1a1a !important;}
...
.streamlit-expanderContent *{color:#1a1a1a !important;}
</style>
...
def generate_llm_report(saju_json: dict, extra: str = ""):
    ...
    model_name = st.session_state.get("gemini_model", "gemini-1.5-flash-latest")
...
"""
    ...
    prompt = f"""당신은 50년 경력의 명리학 대가이자 따뜻한 인생 상담가입니다.
아래 사주 데이터를 바탕으로 매우 입체적이고 풍부한 종합 해석 리포트를 작성해 주세요.

═══════════════════════════════════
{core}
═══════════════════════════════════

다음 12개 영역을 **각각 구체적으로** 작성하세요. 일반인도 이해할 수 있도록 전문 용어를 사용할 때는 괄호로 풀어 설명하세요.

═══════════════════════════════════
## 📜 1. 타고난 본질 (일주 분석)
일간({r["ilgan"]["stem"]})의 본질적 성격, 강점, 약점. 일지({r["pillars"]["day"]["branch"]})가 보여주는 내면의 모습. 어린 시절부터 형성된 기질. 어떤 사람인지 구체 묘사. (200자)

## ⚖️ 2. 오행 균형과 보완 방향
다섯 가지 기운(목·화·토·금·수) 중 무엇이 과하고 부족한지. 일상에서 어떻게 보완할지(음식·환경·습관). (200자)

## 🏛️ 3. 격국이 말하는 인생 큰 그림
{r["yukguk"]}의 의미. 이 사람이 살아가는 주된 방식. 어떤 무대에서 빛나는지. (200자)

## 🔑 4. 용신 — 인생 핵심 키워드
용신({r["yongshin"].get("용신","")})이 왜 중요한가. 이 기운을 어떻게 강화할지. 직업·환경·인테리어 조언. (200자)

## 💰 5. 재물·경제 운
재성(財星) 구조 분석. 평생 재물 흐름. 사업 vs 직장, 투자 성향, 큰 돈을 만질 시기. 어떤 분야에서 돈이 들어오는지. (250자)

## 💼 6. 직업·승진 운
관성(官星)과 식상(食傷) 분석. 적합한 직군, 승진 시기, 직장 내 인간관계, 창업 가능성. 30·40·50대 커리어 흐름. (250자)

## ❤️ 7. 인간관계와 결혼·연애 운
배우자궁({r["pillars"]["day"]["branch"]}) 해석. 어떤 배우자를 만나는지. 결혼 적령기. {"배우자궁 충(沖)이 있어 부침 주의. " if spouse_warn else ""}현재 인연 상황 추측. (250자)

## 🏥 8. 건강 주의사항
오행 부족({r["career_health"]["약한오행"]})으로 인한 신체 약점. 평생 주의할 질환, 현재 대운에서 특히 신경 쓸 부위, 음식·운동 조언. (200자)

## 🌊 9. 현재 대운 흐름 ({cd['age']}~{cd['age']+9}세)
대운 {cd["stem"]}{cd["branch"]}이 가져오는 기운. 이 10년 동안의 주요 키워드. 무엇을 시작하고, 무엇을 정리할 시기인지. (250자)

## 📅 10. 올해({cur_year}년) 1년 운세 + 월별 주요 흐름
올해 전체 운세 톤(상승/조정/주의). 가장 좋은 달, 가장 조심할 달. 분야별(재물·연애·건강·이동) 월별 포인트. 향후 8개월 흐름 구체적으로. (350자)

## 🔮 11. 내년~3년 미래 전망
{cur_year+1}년·{cur_year+2}년 큰 흐름. 어떤 변화·기회·시련이 오는지. 미리 준비할 것. (250자)

## ⭐ 12. 종합 조언 & 활용 팁
이 사주의 빛나는 강점 한 줄. 평생 마음에 새길 좌우명 같은 조언. 행운의 색({", ".join(r["career_health"]["행운색상"][:2])})·방위({r["career_health"]["행운방위"]})·숫자({r["career_health"]["행운숫자"]}) 활용법. (200자)

═══════════════════════════════════

【작성 원칙】
- 친절하고 따뜻한 어조, 존댓말
- 명리 용어 사용 시 반드시 괄호로 풀이 (예: "관성(官星, 직장·명예의 기운)")
- 구체적이고 실용적인 조언 위주
- 미신적이지 않게, 통계·심리학적 관점 곁들이기
- 각 영역 마크다운 헤더 그대로 사용
- 총 분량 3000자 이상의 풍부한 리포트로 작성
"""
...
def main():
    st.markdown(
        ...
        '<div class="sub">호빵사주방 v3.1 · Dynamic Multi-Slot · On-Demand AI</div>'
    ...
    with st.sidebar:
        st.markdown(
            ...
            'font-weight:700;color:#c0392b">🍞 호빵사주방</div>'
        ...
        st.selectbox("모델 선택",
                     ...
                     help="추천: gemini-1.5-flash-latest")
